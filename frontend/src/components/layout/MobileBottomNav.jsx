...
export const MobileBottomNav = ({ onOpenMenu }) => {
  ...
  const isCurrent = (path) => {
    if (path === '/dashboard' || path === '/superadmin/dashboard') {
    ...
    <nav className="fixed bottom-0 left-0 right-0 z-40 lg:hidden bg-white/95 backdrop-blur-md border-t border-slate-200/80 shadow-[0_-4px_20px_rgba(0,0,0,0.06)] px-2 py-1.5 safe-bottom">
      <div className="flex items-center justify-around max-w-lg mx-auto">
        {isSuperAdmin ? (
          ...
          <>
            ...
            <NavLink
              ...
              className={`flex flex-col items-center justify-center py-1 px-2.5 rounded-xl transition-all duration-200 active:scale-95 ${
                isCurrent('/superadmin/dashboard')
                  ? 'text-amber-600 font-bold'
                  : 'text-slate-500 hover:text-slate-800'
              }`}
            >
              <div className="relative">
                <ShieldCheck className={`w-5 h-5 ${isCurrent('/superadmin/dashboard') ? 'stroke-[2.25]' : 'stroke-[1.75]'}`} />
                {isCurrent('/superadmin/dashboard') && (
            ...
            <NavLink
              ...
              className={`flex flex-col items-center justify-center py-1 px-2.5 rounded-xl transition-all duration-200 active:scale-95 ${
                isCurrent('/superadmin/agencies') && !location.search.includes('new=true')
                  ? 'text-amber-600 font-bold'
                  : 'text-slate-500 hover:text-slate-800'
              }`}
            >
              <div className="relative">
                <Building2 className={`w-5 h-5 ${isCurrent('/superadmin/agencies') && !location.search.includes('new=true') ? 'stroke-[2.25]' : 'stroke-[1.75]'}`} />
                {isCurrent('/superadmin/agencies') && !location.search.includes('new=true') && (
                  <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-amber-600 rounded-full" />
                )}
              </div>
              <span className="text-[10px] tracking-tight mt-1">Agencies</span>
            </NavLink>
            ...
            >
              <div className="w-12 h-12 rounded-full bg-gradient-to-tr from-amber-500 to-amber-600 text-slate-950 flex items-center justify-center shadow-lg shadow-amber-500/30 border-2 border-white ring-2 ring-amber-100">
                <Plus className="w-6 h-6 stroke-[2.5]" />
              </div>
              <span className="text-[10px] font-bold text-amber-700 tracking-tight mt-0.5">Add</span>
            </NavLink>
            ...
            <NavLink
              to="/superadmin/users"
              className={`flex flex-col items-center justify-center py-1 px-2.5 rounded-xl transition-all duration-200 active:scale-95 ${
                isCurrent('/superadmin/users')
                  ? 'text-amber-600 font-bold'
                  : 'text-slate-500 hover:text-slate-800'
              }`}
            >
              <div className="relative">
                <UserCheck className={`w-5 h-5 ${isCurrent('/superadmin/users') ? 'stroke-[2.25]' : 'stroke-[1.75]'}`} />
                {isCurrent('/superadmin/users') && (
                  <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-amber-600 rounded-full" />
                )}
              </div>
              <span className="text-[10px] tracking-tight mt-1">Users</span>
            </NavLink>
          ...
          /* ======================================================== */
          /* AGENCY ADMIN BOTTOM NAVIGATION                           */
          /* ======================================================== */
          <>
            {/* 1. Dashboard */}
            <NavLink
